...
% Select the network area %

zone_str = 'zoneC';

% Load datasets %

hyd_data_file = '2018_SCADA_BattleDIM1_1.mat';
...
% Extract hydraulic information for Zone C %

sensors_c_notank = sensors(1:end-1);
for s=1:length(sensors_c_notank)
    index_sensors(s) = find(sensors_comp==sensors_c_notank(s));
end

pressureRaw = pressureRaw(:,index_sensors)';
demandsRaw = demandsRaw'/(1000*3600);
hN_total = pressureRaw+d.NodeElevations(sensors_c_notank)';
hN_total(length(sensors_a),:) = levelsRaw'+d.NodeElevations(reservoirsID_comp(end))'; % add tank head

idk = cell2mat(cellfun(@(x) str2double(strrep(x, 'n', '')), demandsTable.Properties.VariableNames, 'UniformOutput', false));
idk = idk(2:end);
for i=1:length(idk)
    idk(i) = find(zoneC==idk(i));
end
alldel = 1:size(demandsRaw,1);
str_nom = 'zoneC.mat';
...
for leak_index = 1:length(leak_index_v)
    ...
    switch leaky_link_ID
        case 257
            initial_datetime_v = datetime(2018,1,25,8,35,0);
        case 31
            initial_datetime_v = datetime(2018,8,3,2,45,0);
        otherwise
...
for t = iniT:st:endT
    ...
    if mode == 1
        ...
        initialStateGuess = [x_leak_orig];
        ymeas = [hN_total(:,t);demandsRaw(alldel,t)];
        ...
        x_leak = HeadUKF(initialStateGuess,ymeas,R,Q,idk,T,invT,Ss,E,Omega,Phi1,200);
